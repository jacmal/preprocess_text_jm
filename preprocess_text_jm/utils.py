...
def _get_stopwords_counts(x):
	st_c = len([t for t in x.split() if t in stopwords])
	return st_c

def _get_hashtag_counts(x):
	ht_c = len([t for t in x.split() if t.startswith('#')])
	return ht_c

def _get_mention_counts(x):
	me_c = len([t for t in x.split() if t.startswith('@')])
	return me_c
...
def _get_emails(x):
	e_mails = re.findall(r'([a-z0-9+._-]+@[a-z0-9+._-]+\.[a-z0-9+_-]+\b)', x)
	counts = len(e_mails)
	return counts, e_mails
...
def _remove_rare_words(x, freq, n=5):
	fn = freq.tail(n)
	x = ' '.join([t for t in x.split() if t not in fn])
	return x
...
def _all_chars_counter(raw_text, char_list=letters):
	'''
...
def _letter_prob(raw_text, char_list=letters):
	'''
	Take text and char string variable and return:
	1. numpy vector of letters probability
	'''
	#letter counter initialization
	letter_counter = np.zeros(len(char_list))
...
def _text_entropy(raw_text, char_list=letters):
	'''
	Take text and char string variables and return:
	1. return single float value
	'''
	#letter counter initialization
	letter_counter = np.zeros(len(char_list))
	...
	letter_prob = letter_counter / sum(letter_counter)
	#
	text_entropy = -sum(letter_prob * np.log2(letter_prob))

	return text_entropy

def _chars_sequence_counter(raw_text, chars_list=letters):
	'''
